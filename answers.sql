...
SELECT orders.id, status, order_total FROM orders JOIN customers ON (orders.customer_id=customers.id) WHERE email = 'pclark74@gmail.com';
...
SELECT melons.common_name, melons.melon_type, order_items.quantity, order_items.unit_price, order_items.total_price FROM melons JOIN order_items ON (order_items.melon_id = melons.id) WHERE order_id = 2725;
